...
const { bodyAsObject, inventoryRequest, normalizeVersionToken, passInventoryResponse } = upstreamHelpers;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const versionToken = normalizeVersionToken(req.headers['x-inventory-version']);
    if (!versionToken) return res.status(428).json({ error: 'inventory_version_required' });
