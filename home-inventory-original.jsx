...
const S = {
  wrap: { fontFamily: "'Vazirmatn', Tahoma, Arial, sans-serif", direction: "rtl", maxWidth: 640, margin: "0 auto", padding: "1rem", color: "var(--color-text-primary)" },
  row: { display: "flex", alignItems: "center", gap: 8 },
  houseTab: (active) => ({
    padding: "7px 18px", borderRadius: 24, border: `0.5px solid ${active ? "var(--color-text-primary)" : "var(--color-border-secondary)"}`,
  ...
  secTab: (active) => ({
    flex: 1, padding: "10px 4px", border: "none", borderBottom: `2px solid ${active ? "var(--color-text-primary)" : "transparent"}`,
    background: "transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 13,
    color: active ? "var(--color-text-primary)" : "var(--color-text-secondary)", fontWeight: active ? 500 : 400,
    display: "flex", flexDirection: "column", alignItems: "center", gap: 3,
  }),
  card: { background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: 12, padding: "11px 14px", display: "flex", alignItems: "center", gap: 8, marginBottom: 8 },
  ...
  iconBtn: (danger) => ({ width: 30, height: 30, border: `0.5px solid var(--color-border-tertiary)`, borderRadius: 8, background: "transparent", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, color: danger ? "#A32D2D" : "var(--color-text-secondary)", fontFamily: "inherit" }),
  addBtn: { width: "100%", padding: "10px", border: "0.5px dashed var(--color-border-secondary)", borderRadius: 12, background: "transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 14, color: "var(--color-text-secondary)", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 4 },
  input: { width: "100%", padding: "9px 12px", border: "0.5px solid var(--color-border-secondary)", borderRadius: 8, fontFamily: "inherit", fontSize: 14, background: "var(--color-background-primary)", color: "var(--color-text-primary)", direction: "rtl" },
  select: { width: "100%", padding: "9px 12px", border: "0.5px solid var(--color-border-secondary)", borderRadius: 8, fontFamily: "inherit", fontSize: 14, background: "var(--color-background-primary)", color: "var(--color-text-primary)", direction: "rtl", cursor: "pointer" },
  primaryBtn: { padding: "9px 20px", borderRadius: 8, border: "none", background: "var(--color-text-primary)", color: "var(--color-background-primary)", fontFamily: "inherit", fontSize: 14, fontWeight: 500, cursor: "pointer" },
  cancelBtn: { padding: "9px 20px", borderRadius: 8, border: "0.5px solid var(--color-border-secondary)", background: "transparent", color: "var(--color-text-secondary)", fontFamily: "inherit", fontSize: 14, cursor: "pointer" },
  pill: (active) => ({ padding: "4px 12px", borderRadius: 20, border: `0.5px solid ${active ? "var(--color-border-secondary)" : "var(--color-border-tertiary)"}`, background: active ? "var(--color-background-secondary)" : "transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 12, color: active ? "var(--color-text-primary)" : "var(--color-text-secondary)", fontWeight: active ? 500 : 400 }),
  navBtn: (active) => ({ padding: "7px 14px", border: `0.5px solid ${active ? "#F09595" : "var(--color-border-secondary)"}`, borderRadius: 20, background: active ? "#FCEBEB" : "transparent", cursor: "pointer", fontFamily: "inherit", fontSize: 13, color: active ? "#A32D2D" : "var(--color-text-secondary)", display: "flex", alignItems: "center", gap: 6, fontWeight: active ? 500 : 400 }),
};
...
export default function App() {
  ...
  if (!data) return (
    <div style={{ ...S.wrap, textAlign: "center", padding: "3rem", color: "var(--color-text-secondary)" }}>
      در حال بارگذاری...
    </div>
  );
  ...
  const openEdit = (item) => { setForm({ name: item.name, qty: item.qty ?? item.quantity ?? 1, unit: item.unit, status: item.status, id: item.id }); setModal("edit"); };
  ...
  if (modal) {
    ...
    return (
      <div style={{ fontFamily: "'Vazirmatn', Tahoma, Arial, sans-serif", direction: "rtl", minHeight: 500, background: "rgba(0,0,0,0.35)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "2rem 1rem" }}>
        <div style={{ background: "var(--color-background-primary)", borderRadius: 16, padding: "1.5rem", width: "100%", maxWidth: 380, border: "0.5px solid var(--color-border-secondary)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem" }}>
            <span style={{ fontSize: 16, fontWeight: 500 }}>{modal === "add" ? "افزودن آیتم" : "ویرایش آیتم"}</span>
            <button onClick={() => setModal(null)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: "var(--color-text-secondary)", lineHeight: 1 }}>✕</button>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 5 }}>نام</label>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 5 }}>تعداد</label>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  <button style={{ width: 30, height: 34, border: "0.5px solid var(--color-border-secondary)", borderRadius: 6, background: "transparent", cursor: "pointer", fontSize: 18, fontFamily: "inherit" }}
              ...
              <div style={{ flex: 1 }}>
                <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 5 }}>واحد</label>
            ...
            <div>
              <label style={{ fontSize: 12, color: "var(--color-text-secondary)", display: "block", marginBottom: 8 }}>وضعیت</label>
                ...
                {STATUSES.map(s => (
                  <button key={s.id} onClick={() => setForm(f => ({ ...f, status: s.id }))}
                    style={{ flex: 1, padding: "8px 4px", borderRadius: 8, fontFamily: "inherit", fontSize: 12, cursor: "pointer",
                      border: form.status === s.id ? `2px solid ${s.color}` : "0.5px solid var(--color-border-tertiary)",
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{ ...S.row, marginBottom: "1rem", flexWrap: "wrap" }}>
        {data.houses.map(h => (
          editHouse === h.id
            ? <input key={h.id} autoFocus value={houseNameDraft}
                style={{ border: "none", borderBottom: "1.5px solid var(--color-text-primary)", background: "transparent", fontFamily: "inherit", fontSize: 14, fontWeight: 500, color: "var(--color-text-primary)", direction: "rtl", outline: "none", minWidth: 100 }}
        ...
        <span style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginRight: "auto" }}>دوبار کلیک = تغییر نام</span>
      </div>
                    ...
                    {SECTIONS.map(sec => {
                      ...
                      return (
                        <div key={sec.id} style={{ marginBottom: 10 }}>
                          <div style={{ fontSize: 11, color: "var(--color-text-tertiary)", marginBottom: 6 }}>{sec.icon} {sec.label}</div>
                          {its.map(item => (
                            <div key={item.id} style={{ ...S.card, justifyContent: "space-between" }}>
                              <span style={{ fontSize: 14, fontWeight: 500 }}>{item.name}</span>
                              <span style={{ fontSize: 12, color: "var(--color-text-secondary)" }}>{item.qty ?? item.quantity} {item.unit}</span>
                            </div>
                          ))}
                        </div>
                      );
        ...
        <div>
          ...
          {shown.length === 0
            ? <div style={{ textAlign: "center", padding: "2.5rem 1rem", color: "var(--color-text-secondary)" }}>
                <div style={{ fontSize: 32, marginBottom: 10, opacity: 0.5 }}>{SECTIONS.find(s => s.id === section)?.icon}</div>
            ...
            : shown.map(item => {
                ...
                return (
                  <div key={item.id} style={S.card}>
                    ...
                    <button style={S.badge(st)} onClick={() => cycleStatus(item.id)} title="کلیک برای تغییر وضعیت">{st.label}</button>
                    <button style={S.iconBtn(false)} onClick={() => openEdit(item)} title="ویرایش">✏️</button>
                    <button style={S.iconBtn(true)} onClick={() => deleteItem(item.id)} title="حذف">🗑️</button>
                  </div>
                );
              })
          }
